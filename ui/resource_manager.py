...
class ResourceTracker(pygame.sprite.Sprite):
    ...
    def __init__(self, name: str, image_loc: str, amount: int = 0):
    ...
    def refresh_display(self) -> Tuple[pygame.Surface, pygame.Rect]:
        display_image = pygame.Surface((400,TOPBAR_HEIGHT), pygame.SRCALPHA)
        display_image.blit(self.image, (0, 0))
        text_available_amount = ResourceTracker.FONT.render(str(self.amount), True, "black").convert_alpha()
        ...
        if self.proposed_reduction > 0:
            text_proposed_reduction = ResourceTracker.FONT.render(f"(-{str(self.proposed_reduction)})", True, "red").convert_alpha()
